...
<div class="container">
	<div class="p-4 border rounded-corner">
		...
		<form>
			...
			<label>
				<input id="signUpOnline" name="signUpOnline" type="checkbox" bind:checked={signUpOnLine} />
			...
			<center>
				<button type="button" class="btn btn-danger" on:click={() => clearEvent()}
					>Clear Event</button
				>
				<button
					type="button"
					class="btn btn-primary"
					on:click={() => createEventBySendingRequestToEvents()}>Create Event</button
				>
			</center>
		</form>
	</div>
...
{#if isSuccess}
	<center>
		<div class="alert alert-success text-center">An event request has been successfully made</div>
	</center>
{/if}
